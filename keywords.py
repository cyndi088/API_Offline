...
def summary():
    ...
    content = mysql.get_content(id)
    mysql.close_sql()
    summary = sum.func(content)
    output = {'summary': summary, 'status': 1}
    # executor.submit(keywords, id)
    return jsonify(output)
...
def surface():
    ...
    content = mysql.get_content(id)
    mysql.close_sql()
    result = bc.surface(content)
    output = {'surface': result, 'status': 1}
    # executor.submit(keywords, id)
    return jsonify(output)
...
def category():
    ...
    content = mysql.get_content(id)
    mysql.close_sql()
    category = bc.category(title, content)
    output = {'category': category, 'status': 1}
    # executor.submit(keywords, id)
    return jsonify(output)
