...
def insert_inspection(data):
    ...
    obj = ElasticObj("localhost:27017", "zhejiang", "sheng_test", "zhejiang", "sheng", "http://192.168.10.123:9200")
    obj.create_index()
    obj.bulk_Index_Data()
    ra1 = RelationAll()
    ra1.open_sql('yfhunt', 'zhejiang', 'sheng_test')
    ra1.input_sql(data['corpName'])
    ra1.close_sql()
    ra2 = RelationAll()
    ra2.open_sql('yfhunt', 'zhejiang', 'sheng_test')
    ra2.input_sql(data['corpNameBy'])
    ra2.close_sql()
    re1 = Relation()
    re1.open_sql('yfhunt', 'zhejiang', 'sheng_test')
    re1.input_sql(data['corpName'])
    re1.close_sql()
    re2 = Relation()
    re2.open_sql('yfhunt', 'zhejiang', 'sheng_test')
